## 2026-08-03 — Add Practice Goal tracker to Dashboard

**What:** Added a `GoalTracker` component to the Dashboard, positioned between the three summary stat cards and the Practice Streak Counter. The widget lets users set a personal target average score (1–100) and instantly see how close their current average is to that goal. When no goal is set, a soft dashed prompt card with a "Set Goal" button invites the user to create one. Clicking it reveals an inline form with a number input, Save, and Cancel buttons; pressing Enter or Escape also work as expected. Once a goal is saved, the panel shows the current average and target side by side with an arrow between them, a labelled horizontal progress bar that fills proportionally from 0% to 100% (capped at 100% so it never overflows), and a percentage progress label on the right. A contextual line below the bar tells the user exactly how many points they are away from their target (or invites them to complete their first session if they have none yet). When the user's current average meets or exceeds the goal, the card switches to an emerald-green "Goal Reached!" celebration state — the border, bar, and labels all turn emerald, a "Goal Reached!" badge appears in the header, and a message suggests raising the goal to the next multiple of five to keep improving. The goal is stored in `localStorage` under the key `interviewGoalScore` so it persists across page reloads and browser sessions without requiring a backend round-trip. An "Edit goal" link in the header reopens the inline form pre-seeded with the current value; from there the user can update the number or click "Remove goal" to clear it entirely. A loading skeleton is shown while sessions are being fetched, matching the `animate-pulse` pattern used by the rest of the Dashboard's data-dependent widgets. No new npm dependencies were added and no backend changes were required.

**Why:** The Dashboard already shows the user's current average score, but there is nothing to aim for — a score of 68% could feel fine or feel like a failure, depending entirely on the user's personal bar. A goal gives that number meaning: "I need 80%" immediately frames 68% as 12 points away, and the progress bar makes that gap tangible. Setting goals is one of the most reliable motivational mechanics in learning tools: it converts a vague intention ("get better at interviews") into a concrete, trackable target that the user can check every time they open the Dashboard. Celebrating the goal reached state with the "Goal Reached!" badge adds a small but meaningful positive-feedback moment and naturally prompts the user to raise the bar and keep improving. Using localStorage keeps the implementation self-contained and instant — there is no loading delay when the goal is already set, and no extra API request on every page view.

**Files changed:**
- `client/src/pages/Dashboard.jsx` — added `Target` to the lucide-react import; added `GoalTracker` functional component (loading skeleton variant, no-goal prompt variant, inline editing form variant, and goal-display variant with progress bar, celebration state, and contextual copy); inserted `<GoalTracker sessions={sessions} loading={fetching} />` between the metrics grid and the StreakCounter

---

## 2026-08-02 — Add "Export CSV" button to Dashboard session history

**What:** Added an "Export CSV" button to the top-right corner of the Interview History panel on the Dashboard. Clicking the button generates a CSV file containing every session the user has ever started — not just the currently visible filtered/paginated page, but the full unfiltered history — and triggers an immediate browser download. The filename includes the current date (e.g. `interview-history-2026-08-02.csv`). Each row includes eleven columns: **Date**, **Role**, **Difficulty**, **Type** (question category), **Tech Stack**, **Status** (Completed / In Progress), **Score** (percentage for completed sessions, blank for in-progress), **Duration** (MM:SS, blank if not recorded), **Questions** (count), **Starred** (Yes/No), and **Notes** (personal notes with newlines flattened to spaces). Fields that contain commas, double-quotes, or newlines are correctly RFC-4180 escaped so the file opens cleanly in Excel, Google Sheets, and Numbers. The button is only shown after sessions have loaded and only when at least one session exists — it stays hidden while the skeleton is animating and for brand-new users with no history. No new npm dependencies were added and no backend changes were required — all data comes from the already-fetched sessions array.

**Why:** The single-session "Download Report" button added to FeedbackView on 2026-07-31 covers the per-session use case, but users who want to analyse trends across dozens of sessions — build a portfolio spreadsheet, share their full history with a mentor, import it into a personal tracking tool, or simply archive it — have no way to do that without manually copying each session card. The CSV export closes that gap in one click. Exporting the complete unfiltered history (rather than just the current filter view) means the file is always a faithful full archive; users can do their own filtering in a spreadsheet tool. This addresses the "UI/UX improvements" priority area and is the natural all-history complement to the per-session download already in FeedbackView.

**Files changed:**
- `client/src/pages/Dashboard.jsx` — added `Download` to the lucide-react import; added `handleExportCSV` function (builds a rows array from `sessions`, escapes fields per RFC 4180, joins into a CSV string, triggers a Blob download); added "Export CSV" button with `Download` icon next to the "Interview History" heading, conditionally rendered when sessions have loaded and the list is non-empty
